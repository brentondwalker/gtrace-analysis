...
object EventDataTransformer {
  ...
  def transformTaskData(spark: SparkSession, df: Dataset[Row]) : Dataset[Row] = {
  ...
  def transformJobData(spark: SparkSession, df: Dataset[Row]) : Dataset[Row] = {
    import spark.implicits._
